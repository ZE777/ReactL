 ...
 *
 * 由 Markdown 的 `pre` 映射在偵測到 ```mermaid 區塊時使用。為 Client Component
 * （需 useEffect 動態載入 mermaid 並繪圖），在分享頁（Server Component）內作為
 * client boundary 渲染，hydration 後才繪圖。
 *
...
function MermaidImpl({ code }: { code: string }) {
  ...
  return (
    <div className="my-3 rounded-lg overflow-hidden border border-zinc-700/60 bg-zinc-950">
      ...
      <div ref={containerRef} className="px-3 py-3 overflow-x-auto bg-white dark:bg-zinc-900 flex justify-center">
        {svg ? (
