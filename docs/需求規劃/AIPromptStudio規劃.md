...
- 列表顯示外部平台（Line / Discord）的對話記錄（來源為 `ExternalMessages`）
- 可依平台、Bot、時間範圍篩選
- 點進去看完整對話內容
- Token 消耗顯示

#### 資料隔離與隱私（實作確認，2026-06-05）

> 重點：監控資料**依使用者隔離**，多人共用系統金鑰**不會**造成對話內容互相可見。

- **依擁有者隔離**：每位使用者只看得到「自己建立的 Bot」的對話與「自己」的 Token 統計。後端 `MonitorService` 三個查詢都硬性過濾 `BotBinding.UserId == 登入者` / `TokenUsageStats.UserId == 登入者`，看不到他人資料。
- **前台公開聊天不留存**：前台聊天室（`/public/chat/stream`）走系統預設金鑰、且**完全不寫入資料庫**（ephemeral），因此不會出現在任何人的監控中。故監控對話來源只有 Line / Discord，沒有 Web。
- **可見性 vs 金鑰來源是兩件事**：對話可見性由「Bot 由誰建立（`BotBinding.UserId`）」決定，**與該次呼叫用了哪把金鑰無關**。即使 Bot 沒設自帶金鑰、fallback 用系統預設金鑰，對話與統計仍只歸屬於 Bot 擁有者。
- **共用的是額度、不是隱私**：多個未設自帶金鑰的使用者，其 Bot 會 fallback 到同一把系統金鑰，因此**共用同一份 rate limit / 額度**（某人 Bot 流量可能吃掉共用配額）——但彼此對話內容互相看不到。若要隔離額度，使用者可於 BYOK（AI 金鑰）頁面設定自帶金鑰。
